...
name <- as.character(paste(filename,"channels_1_4.png"))
name <- sub(" ","",name)
png(name)
p <- plotClusters(X.trans[,1:4],classification=res$cluster, chulls=FALSE, ellipses=TRUE)
print(p)
dev.off()

name <- as.character(paste(filename,"channels_5_8.png"))
name <- sub(" ","",name)
png(name)
p <- plotClusters(X.trans[,5:8],classification=res$cluster, chulls=FALSE, ellipses=TRUE)
print(p)
dev.off()

name <- as.character(paste(filename,"channels_9_12.png"))
name <- sub(" ","",name)
png(name)
p <- plotClusters(X.trans[,9:12],classification=res$cluster, chulls=FALSE, ellipses=TRUE)
print(p)
dev.off()

name <- as.character(paste(filename,"channels_13_16.png"))
name <- sub(" ","",name)
png(name)
p <- plotClusters(X.trans[,13:16],classification=res$cluster, chulls=FALSE, ellipses=TRUE)
print(p)
dev.off()
